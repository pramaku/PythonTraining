...
def ip_addr_class_ver2(*ip_addrs):
    result = {}
    result['valid'] = []
    result['invalid'] = []
    result['classA'] = []
...
total_int = raw_input('enter the count of ip addrs')
ip_addrs = []
for x in range(int(total_int)):
    ip_addrs.append(raw_input('enter ip addr: '))

op = ip_addr_class_ver2(*ip_addrs)
class_list = ['valid', 'classA','classB', 'classC',  'classD', 'invalid']
for class_type in class_list:
    print class_type, ' : '
    for value in op[class_type]:
        print '-- ', value
    print '-' * 60
